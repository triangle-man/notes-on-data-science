...
complicated thing: in fact, it is \emph{linear}
...
That is, the derivative at $\vv{x}$ actually ``lives in'' the space
$\mathcal{L}(\V,\set{R})$, the space of linear maps from $\V$
to~$\set{R}$. And this space is much (much) smaller than the full
space~$\set{R}^{\V}$.

The space $\mathcal{L}(\V,\set{R})$ is known as the \emph{dual}
of~$\V$, written $\V^*$.  It is itself a vector space and in fact has
the same dimension as~$\V$.  In other words, there is some dual
vector, $\vv{G}\in\V^*$, such that
$\nabla_{\vv{\xi}}f = \vv{G}(\vv{\xi})$. (I am omitting the dependency on
$\vv{x}$.) This $\vv{G}$ is called the \emph{gradient} of~$f$; it is
normally writen as simply $\nabla f$.

Note that the gradient of a function, $f$, is not an element of $\V$,
it is an element of $\V^*$.  If one happens to represent an element of
$\V$ as a tuple of numbers, then one might represent an element of
$\V^*$ as a tuple of numbers of the very same length: but they are not
the same thing.\sidenote{People often visually distinguish the two
...
\begin{enumerate}
\item The derivative of a function $f\colon\R\to\R$ is a number at each
  point; that is, it too is a function $\R\to\R$.
\item The derivative of a function $f\colon \R\to\W$ is, at each point,
  a vector in~$\W$; that is, it is a function $\R\to\mathcal{W}$.
\item The derivative of a function $f\colon\V\to\R$ is 
\end{enumerate}

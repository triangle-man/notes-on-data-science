...
operation available in~$\W$. The result of the right-hand side is a
vector in~$\W$ and so the derivative is a vector in~$\W$. 
...
derivative of~$\vv{f}$.\sidenote{The dot notation is normally used
  ...
  notation for the derivative is~$\vv{f}'$.} Since $\dot{\vv{f}}$
``lives in'' $\W$, we can also imagine that, to each point on the path
traced out by $\vv{f}$ in $\W$, we assign the vector~$\dot{\vv{f}}(t)$
(see figure~\ref{fig:velocity}).  If $\vv{f}(t)$ is a ``position at
time $t$'', then $\dot{\vv{f}}(t)$ might be thought of as ``the
velocity at time~$t$.''
...
however, the derivative is \emph{linear} in~$\vv{\xi}$.\sidenote{To show
  ...
  \beta\nabla_{\vv{\zeta}} f$ for any two numbers
...
$\nabla_{\vv{\xi}}f = \vv{G}(\vv{\xi})$. This $\vv{G}$ is called the ``gradient of
...
thought of as a vector in~$\V$.  But we have just concluded that the
gradient, $\nabla f$, is \emph{not} an element of~$\V$; it is, instead, an
element of $\mathcal{L}(\V, \R)$, the space of linear maps from $\V$
to~$\R$.  Those are not the same thing.  What is going on?
...
From now on, I will not use boldface to distinguish vectors from real
numbers.

\emph{Example:} Suppose $f$ is a constant function, say $f(x)=c$,
where $c\in\W$, for any~$x\in\V$.  Then $\nabla f$ is, at each point $x\in\V$,
the zero map, $\nabla f\colon v\mapsto 0$.

\emph{Example} Suppose $f$ is itself a linear map, that is
$f\in\mathcal{L}(\V,\W)$.  Consider the derivative on the right-hand side of
eq.~\eqref{eq:gradient} at some point $x\in\V$ and given a direction
$\xi\in\V$.  Since $f$ is linear, we have
\begin{equation*}
  \begin{aligned}
